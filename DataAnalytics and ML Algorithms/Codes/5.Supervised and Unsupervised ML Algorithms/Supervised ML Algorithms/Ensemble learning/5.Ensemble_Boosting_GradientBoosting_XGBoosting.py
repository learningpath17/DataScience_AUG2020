...
#ada boost - try to give more weight for wrongly predicted samples for next iteration(model)
dt_estimator=tree.DecisionTreeClassifier()
ada_estimator = ensemble.AdaBoostClassifier(base_estimator=dt_estimator,random_state=100)
...
#gradient boosting - try to reduce the residual errors for next model building process.
...
#xgb = boosting + regulaization for overfit control (pip install xgboost)
...
titanic_test.to_csv("submission_XGBoosting_01.csv", columns=["PassengerId", "Survived"], index=False)